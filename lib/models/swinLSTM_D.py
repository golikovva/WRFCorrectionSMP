...
class SwinLSTM(nn.Module):
    def __init__(self, img_size, patch_size, in_chans, embed_dim, depths_downsample, depths_upsample, num_heads,
                 window_size, out_chans=3, target_len=1):
        super(SwinLSTM, self).__init__()
        print(depths_downsample)
        ...
        self.Upsample = UpSample(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
                                 ...
                                 num_heads=num_heads, window_size=window_size)
        self.num_layers = depths_downsample
        self.target_len = target_len
        self.out_chans = out_chans

    def single_forward(self, input, states_down, states_up):
    ...
    def forward(self, inputs, targets_len=None):
        states_down = [None] * len(self.num_layers)
        states_up = [None] * len(self.num_layers)
        ...
        for i in range(inputs_len - 1):
            output, states_down, states_up = self.single_forward(inputs[:, i], states_down, states_up)
            # outputs.append(output)
        targets_len = targets_len if targets_len is not None else self.target_len
        for i in range(targets_len):
            output, states_down, states_up = self.single_forward(last_input, states_down, states_up)
            outputs.append(output)
            last_input = output
        return torch.stack(outputs, 1)[:, :, :self.out_chans]
