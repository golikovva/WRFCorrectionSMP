...
class ViT(nn.Module):
    ...
    def forward(self, x, lead_times=None, film_index=None):
        ...
        # print(lead_times.shape, 'lead times shape')
        out_transformers = self.forward_encoder(x, lead_times[:, 0], self.default_vars)
        # print(out_transformers.shape, 'transformer output shape')
        preds = self.head(out_transformers)
        # print(preds.shape, 'head output shape')
        preds = self.unpatchify(preds)
        # print(preds.shape, 'unpatchified output shape')
