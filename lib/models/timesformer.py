...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)
        W = x.size(-1)
        # print(x.shape, 'patched')
        x = x.flatten(2).transpose(1, 2)
        # print(x.shape , 'patch flattened')
        return x, T, W
...
class VisionTransformer(nn.Module):
    """ Vision Transformere
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, out_chans=3, embed_dim=768, depth=12, decoder_depth=4,
    ...
    def forward_features(self, x):
        ...
        # print(x.shape, 'self.patch_embed(x)')
        # cls_tokens = self.cls_token.expand(x.size(0), -1, -1)
        # x = torch.cat((cls_tokens, x), dim=1)
        # print(x.shape, 'with cls token')

        # print(x.size(1),  self.pos_embed.size(1))
        ## resizing the positional embeddings in case they don't match the input at inference
        if x.size(1) != self.pos_embed.size(1):
            pos_embed = self.pos_embed
            # cls_pos_embed = pos_embed[0,0,:].unsqueeze(0).unsqueeze(1)
            other_pos_embed = pos_embed[0,:,:].unsqueeze(0).transpose(1, 2) #[0,1:,:]
            ...
            new_pos_embed = torch.cat((new_pos_embed,), 1) #cls_pos_embed, new_pos_embed
            x = x + new_pos_embed
        else:
            x = x + self.pos_embed
        x = self.pos_drop(x)
        # print(x.shape, 'self.pos_drop(x)')
        ...
        if self.attention_type != 'space_only':
            ...
            x = torch.cat((x,), dim=1)  # (cls_tokens, x)
        
        # print(x.shape, 'berfore self.blocks(x)')
        ## Attention blocks
        for blk in self.blocks:
            x = blk(x, B, T, W)
        # print(x.shape, 'self.blocks(x)')
...
class TimeSformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, out_chans=3, embed_dim=768, num_frames=8, depth=12, decoder_depth=4, num_heads=12, mlp_ratio=4, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        super(TimeSformer, self).__init__()
        self.pretrained=True
        self.model = VisionTransformer(img_size=img_size, in_chans=in_chans, out_chans=out_chans, patch_size=patch_size, embed_dim=embed_dim, depth=depth, decoder_depth=decoder_depth, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=num_frames, attention_type=attention_type, **kwargs)
