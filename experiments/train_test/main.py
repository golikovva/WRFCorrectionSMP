...
from lib.data.datasets import WRFs2sDataset, ERAs2sDataset, ERAMonthlyDataset, ScatterDataset, StationsDataset, StackDataset, \
    dataset_with_indices, StationsDataset, ScatterDataset, ConcatDataset
...
def main(cfg):
    ...
    era_dataset_uv = ERAMonthlyDataset(os.path.join(cfg.data.era_folder, 'w10'), ['u10', 'v10'], seq_len=max_sl, region_bbox=[55, 90, -180, 180])
    era_dataset_t = ERAMonthlyDataset(os.path.join(cfg.data.era_folder, 't2'), ['t2m'], seq_len=max_sl, region_bbox=[55, 90, -180, 180])
...
if __name__ == '__main__':
    ...
    default_cfg = Config.fromfile(osp.join('/home/configs/', 'config.yaml'))
